...
async function loadSubmissions() {
  ...
  try {
    ...
    const { getAllUsers: getUsrs } = await import("../firebase/database.js");
    const users = await getUsrs();
    const userMap = {};
    ...
    list.innerHTML = subs.map(s => {
      ...
      const photo = u.photoURL ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(u.nickname||u.username||"U")}&background=1a1a2e&color=c9a84c`;
      return `
      <div class="submission-item" id="sub-${s.id}">
        <div class="submission-user">
          <img src="${photo}" class="submission-avatar"
        ...
        <div class="submission-actions">
          ${s.printUrl ? `<button class="btn-secondary btn-view-print" data-url="${s.printUrl}"
...
async function loadAdminQuests() {
  ...
  try {
    ...
    const { getDatabase, ref: dbRef, get: dbGet } = await import(
      "https://www.gstatic.com/firebasejs/12.10.0/firebase-database.js"
    );
    const { db }      = await import("../firebase/services-config.js");
    const { snapToArray } = await import("../firebase/database.js");
    const snap   = await dbGet(dbRef(db, "quests"));
    ...
    if (quests.length === 0) {
      list.innerHTML = `<div class="empty-state"><i class="fas fa-scroll"></i>
        <h3>Nenhuma quest criada</h3><p>Clique em "Nova Quest"!</p></div>`;
      return;
    }
    ...
    list.querySelectorAll(".btn-edit-quest")  .forEach(b => b.addEventListener("click", () => openQuestModal(b.dataset.id, quests)));
...
function openQuestModal(questId, quests) {
  ...
  if (questId && quests?.length) {
    ...
    titleEl.innerHTML = '<i class="fas fa-plus"></i> Nova Quest';
...
document.getElementById("closeQuestModal")  ?.addEventListener("click", () => { document.getElementById("questModal").style.display = "none"; });
document.getElementById("cancelQuestModal") ?.addEventListener("click", () => { document.getElementById("questModal").style.display = "none"; });
...
async function doToggleQuest(id) {
  try {
    ...
    window.showToast?.(`Quest ${isNowActive ? "ativada" : "desativada"}!`, "info");
    await loadAdminQuests();
  } catch (err) {
...
async function doDeleteQuest(id) {
  if (!confirm("Tem certeza que deseja deletar esta quest?")) return;
  try {
...
async function loadUsers() {
  ...
  try {
    ...
    tbody.innerHTML = users.map(u => {
      const photo = u.photoURL ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(u.username||"U")}&background=1a1a2e&color=c9a84c&size=32`;
      return `
      <tr>
        <td>
          <div class="table-user-cell">
            <img src="${photo}" alt="" class="table-avatar"
                 onerror="this.src='https://ui-avatars.com/api/?name=?&background=1a1a2e&color=c9a84c'"/>
            <div>
...
window.doToggleRole = async (uid, currentRole) => {
  ...
  if (!confirm(`${newRole === "admin" ? "Tornar admin?" : "Remover admin?"}`)) return;
  try {
